    ...
    public function grouppay()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            ...
            if (!empty($row_counter)) {
                ...
                } else if ($pay_method->payment_type == "ompay") {
                    if ($pay_method->api_publishable_key == "") {
                        $this->session->set_flashdata('error', '<div class="alert alert-danger">' . $this->lang->line('ompay_settings_not_available') . '</div>');
                        redirect($_SERVER['HTTP_REFERER']);

                    } else {

                        $this->session->set_userdata("params", $params);
                        redirect(base_url("user/gateway/ompay"));
                    }
        ...
        if (!empty($this->payment_method)) {
            ...
            if ($pay_method->payment_type == "paypal") {
                ...
                if ($pay_method->api_publishable_key == "") {
                    ...
                    $this->session->set_userdata("params", $params);
                    redirect(base_url("students/ompay"));
                }
